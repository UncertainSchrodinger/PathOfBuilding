...
fn con_execute(_ctx: Context, command: String) -> Result<()> {
    println!("Execute received {}", command);
    Ok(())
}

// TODO: Is this supposed to be connection execute or?
fn set_window_title(_ctx: Context, title: String) -> Result<()> {
    println!("Title received {}", title);
    Ok(())
}

fn get_time(_ctx: Context, _: ()) -> Result<()> {
    println!("Get Time called");
    Ok(())
}

fn set_main_object(_ctx: Context, table: Table) -> Result<()> {
    println!("Got set main table {:?}", table);
    Ok(())
}

fn render_init(_ctx: Context, _: ()) -> Result<()> {
    println!("Render init called");
    Ok(())
}

fn con_printf(_ctx: Context, message: String) -> Result<()> {
    println!("Printing {}", message);
    Ok(())
}

fn pload_module(ctx: Context, module: String) -> Result<()> {
    ctx.load(&format!("require('{}')", &module)).exec()?;
    println!("pload module called {}", module);
    Ok(())
}

fn show_err_message(_ctx: Context, message: String) -> Result<()> {
...
impl PathOfBuilding {
    pub fn start(self) -> Result<()> {
        ...
        lua.context(|ctx| {
            ...
            let function_con_execute = ctx.create_function(con_execute)?;
            globals.set("ConExecute", function_con_execute)?;

            let function_window_title = ctx.create_function(set_window_title)?;
            globals.set("SetWindowTitle", function_window_title)?;

            let function_set_main_object = ctx.create_function(set_main_object)?;
            globals.set("SetMainObject", function_set_main_object)?;

            let function_get_time = ctx.create_function(get_time)?;
            globals.set("GetTime", function_get_time)?;

            let function_render_init = ctx.create_function(render_init)?;
            globals.set("RenderInit", function_render_init)?;

            let function_con_printf = ctx.create_function(con_printf)?;
            globals.set("ConPrintf", function_con_printf)?;

            let function_pload_module = ctx.create_function(pload_module)?;
            let function_load_module = ctx.create_function(pload_module)?;
            globals.set("PLoadModule", function_pload_module)?;
            globals.set("LoadModule", function_load_module)?;

            let function_show_err_msg = ctx.create_function(show_err_message)?;
        ...
        lua.context(|lua_ctx| {
            ...
            lua_ctx
                .load("package.path = package.path .. ';./src/?.lua' .. ';./runtime/lua/?.lua'")
                .exec()?;

            // require a module located in the newly added directory
            lua_ctx.load("require('Launch')").exec()?;
